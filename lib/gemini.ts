...
export const generateInterviewQuestions = async (context: string): Promise<string[]> => {
  try {
    ...
    const prompt = `Generate 6 technical interview questions in valid JSON formate based on: ${context}
    ...
    if (!parsed.questions || !Array.isArray(parsed.questions)) {
      throw new Error("Invalid question formate from API");
    };
...
export const analyzeResponse = async (question: string, answer: string): Promise<AnalysisResult> => {
  try {
    ...
    const prompt = `Analyze this interview response (1-100 score) considering:
      ...
      if (typeof parsed.score !== 'number' || typeof parsed.technicalFeedback !== 'string' || typeof parsed.communicationFeedback !== 'string' || !Array.isArray(parsed.improvementSuggestions)) {
        throw new Error('Invalid analysis formate from API');
      }
...
export const generateInterviewFeedback = async (interview: any): Promise<FeedbackResult> => {
  try {
    const model = genAI.getGenerativeModel({
      model: "gemini-2.0-flash",
      generationConfig,
