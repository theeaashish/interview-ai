...
const getFallBackQuestions = (jobRole: string, techStack: string[]): string[] => {
  const role = jobRole.toLowerCase();
  const stack = techStack.map(tech => tech.toLowerCase());

  if (role.includes('frontend') || stack.some(tech => 
  ['react', 'vue', 'angular', 'javascript', 'typescript', 'html', 'css'].includes(tech))) {
    return DEFAULT_QUESTIONS.frontend;
  }

  if (role.includes('backend') || stack.some(tech => 
    ['node', 'express', 'django', 'flask', 'spring', 'java', 'python', 'c#', '.net'].includes(tech))) {
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error('Failed to generate questions with gemini api, using fall back questions: ', error);

      // check if it's rate limit error
      const isRateLimitError = error.message && (
        error.message.includes('429') || error.message.includes('Too Many Requests') || error.message.includes('RATE_LIMIT_EXCEEDED')
      )
    ...
    return NextResponse.json({ message: usedFallBack ? 'interview created with default questions due to AI service limitation. Try again later for personalized questions.' : 'interview created successfully', interview: newInterview },  { status: 201 });
    ...
    if (error.message && error.message.includes('rate limit') ||
        ...
        error.message && error.message.includes('Too Many Requests')) {
          errorMessage = 'AI service is currently busy. Please try again in few minutes.';
          statusCode = 429;
        } else if (error.message && error.message.includes('failed to generate interview questions')) {
          errorMessage = 'Unable to generate interview questions. Please try again later.';
        }
